...
async function logUserAction(userId, action, targetTable, targetId, details) {
    try {
        await db.execute({
            sql: `INSERT INTO USER_AUDIT_LOG (user_id, action, target_table, target_id, details) 
                  VALUES (?, ?, ?, ?, ?)`,
            args: [userId, action, targetTable, targetId, details]
        });
...
async function logGuardianAction(guardianId, action, targetTable, targetId, details) {
    try {
        await db.execute({
            sql: `INSERT INTO GUARDIAN_AUDIT_LOG (guardian_id, action, target_table, target_id, details) 
                  VALUES (?, ?, ?, ?, ?)`,
            args: [guardianId, action, targetTable, targetId, details]
        });
...
async function logAdminAction(adminId, action, targetTable, targetId, details) {
    try {
        await db.execute({
            sql: `INSERT INTO ADMIN_AUDIT_LOG (admin_id, action, target_table, target_id, details) 
                  VALUES (?, ?, ?, ?, ?)`,
            args: [adminId, action, targetTable, targetId, details]
        });
